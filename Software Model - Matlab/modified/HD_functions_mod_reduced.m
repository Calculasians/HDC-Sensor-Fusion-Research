...
function [numPat, AM] = hdctrainproj (classes,labelTrainSet1, trainSet1, trainSet2, trainSet3, CiM, iM1, iM2, iM3, D, N, precision, channels1, channels2, channels3,projM1_pos, projM1_neg, projM2_pos, projM2_neg, projM3_pos, projM3_neg ) 
%
...
%
    ...
    while i <= length(labelTrainSet1)-N+1
       	if labelTrainSet1(i) == label
            ...
            if labelTrainSet1(i) == label && (labelTrainSet1(i+1) ~= label || labelTrainSet1(i+2) ~= label)
                i = i + 1;
            else
...
%
    ...
    for i = 1:1:length(testSet1)-N+1        
	    ...
	    [predict_hamm, error, second_error, dist] = hamming(sigHV, AM, classes);
        distance = [distance; dist];
        predicted = [predicted; predict_hamm];
    ...
    writematrix(predicted,'predicted.csv');
...
function [predict_hamm, error, second_error, dist] = hamming (q, aM, classes)
%
