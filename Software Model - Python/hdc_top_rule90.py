...
class HDCTop:
	...
	def __init__(self, 	fm_infile, seed_hv_infile, is_early_fusion=True, use_3_seed=False):
	...
	def run_spatial_encoder(self, features):
		if (self.use_3_seed):
			im = self.seed_hv.getRow(0).tolist();
			projm_pos = self.seed_hv.getRow(1).tolist();
			projm_neg = self.seed_hv.getRow(2).tolist();
		else:
			im = self.seed_hv.getRow(0).tolist();
			projm_pos = utils.gen_next_hv_rule_90(im);
			projm_neg = utils.gen_next_hv_rule_90(projm_pos);
		...
		for i in range(max(self.num_channel_GSR, self.num_channel_ECG, self.num_channel_EEG)):
			...
			if (i < self.num_channel_GSR):
				...
				self.spatial_encoder_GSR.bind(i, input_Bi, im)
			...
			if (i < self.num_channel_ECG):
				if (features[i+self.num_channel_GSR] == 1):
					...
					input_Bi = [0] * self.dimension
				self.spatial_encoder_ECG.bind(i, input_Bi, im)
			...
			if (i < self.num_channel_EEG):
				if (features[i+self.num_channel_GSR+self.num_channel_ECG] == 1):
					...
					input_Bi = [0] * self.dimension
				self.spatial_encoder_EEG.bind(i, input_Bi, im)

			if (self.use_3_seed):
				im = utils.gen_next_hv_rule_90(im);
				projm_pos = utils.gen_next_hv_rule_90(projm_pos);
				projm_neg = utils.gen_next_hv_rule_90(projm_neg);
			else:
				im = utils.gen_next_hv_rule_90(projm_neg);
				projm_pos = utils.gen_next_hv_rule_90(im);
				projm_neg = utils.gen_next_hv_rule_90(projm_pos);
