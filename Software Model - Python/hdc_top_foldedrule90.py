...
class HDCTop:
	...
	def run_spatial_encoder(self, features):
		...
		self.im = [utils.gen_next_hv_folded_rule_90(self.projm_neg, self.num_folds)]
		...
		for i in range(max(self.num_channel_GSR, self.num_channel_ECG, self.num_channel_EEG)):
			...
			self.im.append(utils.gen_next_hv_folded_rule_90(self.im[len(self.im)-1], self.num_folds))
